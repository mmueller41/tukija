...
Paddr Cip::map(Pd *parent, Pd *self, Paddr parent_va)
{
    ...
    self->Space_mem::insert(self->quota, (USER_ADDR - PAGE_H_SIZE - PAGE_T_SIZE - PAGE_SIZE - 4*PAGE_SIZE), 2, Hpt::HPT_U | Hpt::HPT_W | Hpt::HPT_P, Buddy::ptr_to_phys(reinterpret_cast<void *>(cip_hva)));
    ...
    mword virt = align_dn(USER_ADDR - PAGE_H_SIZE - PAGE_T_SIZE - PAGE_SIZE - 4*PAGE_SIZE, PAGE_SIZE);
    ...
    trace(TRACE_TIP, "Mapped CIP at %lx", (USER_ADDR - PAGE_H_SIZE - PAGE_T_SIZE - PAGE_SIZE - 4 * PAGE_SIZE));
